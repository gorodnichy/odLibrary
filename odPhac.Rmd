...
```{r  include=FALSE}
# phac-data.Rmd = odPhac_covid19.Rmd code to generate
# 1.1 odCovid.by.age = odPhac_covid19-epiSummary-ageGender.Rds (includes populations)
# 1.2 odCovid.by.province =  (includes populations)
# 2 odPhac_vaccination-coverage-byAgeAndSex-overTimeDownload (includes populations)
# 3.1 odPhac_adverse.reactions.Rds

knitr::opts_chunk$set(echo = F, message=F, warning=F)
setwd("../OpenCanada.info")
...
odCovid.by.age <- COpenData$new(
  ...
  info =  c (
    ...
    # source = "PHAC Epi Update:  Covid Cases/Deaths",
    title = "By age and gender, relative to population size, all provinces combined",
    title.short = "COVID-19 Deaths -- by age and gender",
    webpage  = "https://health-infobase.canada.ca/covid-19",
    comment.priv = "Figure 3. Weekly number of COVID-19 cases deceased by age group in Canada as of January 21, 2023"
    ), 
  data =  c(
    origin="csv.live",
    csv.live="https://health-infobase.canada.ca/src/data/covidLive/covid19-epiSummary-ageGender.csv",
    csv.local = "covid19-epiSummary-ageGender.csv"
  )
)
...
if (T) { # read and store.me()
  if (T) { # C_OPEN_DATA_USE_LOCAL) {
    self %>% od.fread("csv.local")
  } else {
    self %>% od.fread("live")
    if (F) {
      self %>% od.fwrite(timestamp = T);   self$fwrite.csv(timestamp = F)
    }
  }
  opendata.view(self)
}
...
self$get.dt() %>% dt.summary(col.factors = self$col.factors, col.metric = self$col.metrics)

self$get.dt.subset() 

self$get.dt() %>% datatable.title() 
self$get.dt.subset() %>% datatable.title() 
dt2dt.with.date.as.character %>% datatable.title() 
